...
Partial Class _Default
    ...
    Public Sub agregaArchivoDeEntrada(ds As DataSet)
        ...
        Try
            ...
            For Each row In ds.Tables(0).Rows
                ...
                If Trim(row(0)).Equals("") Then
                    Continue For
                End If

                tipoDeOrden = getValueFile(conn, row, "tipoDeOrden") '= "'Corte'" 'parseCodigo(conn, tipoDeOrden)

                If tipoDeOrden.Equals("'10'") Then
                    descOrden = "Corte"
                    tipoDeOrden = "'TO002'"
                ElseIf tipoDeOrden.Equals("'60'") Then
                    descOrden = "Reconexión"
                    tipoDeOrden = "'TO003'"
                ElseIf tipoDeOrden.Equals("'90'") Then
                    descOrden = "Instalación"
                    tipoDeOrden = "'TO005'"
                ...
                Dim marcaMedidor As String = ""

                'numMedidor = numMedidor.Substring(1, numMedidor.Length - 2)


                'While Not IsNumeric(numMedidor.Substring(0, 1))
                '    marcaMedidor += numMedidor.Substring(0, 1)
                '    numMedidor = numMedidor.Substring(1)
                'End While
                'numMedidor = "'" & numMedidor & "'"
                'marcaMedidor = "'" & marcaMedidor & "'"

                Dim calle As String = getValueFile(conn, row, "calle")
                calle = calle.Substring(1, calle.Length - 2)
                Dim numInterior As String = ""
                If calle.IndexOf("#") >= 0 Then
                    numInterior = calle.Substring(calle.IndexOf("#") + 1)
                    calle = Trim(calle.Substring(0, calle.IndexOf("#") - 1))
                ElseIf calle.IndexOf("S/N") >= 0 Then
                    numInterior = calle.Substring(calle.IndexOf("S/N") + 1)
                    calle = Trim(calle.Substring(0, calle.IndexOf("S/N") - 1))
                End If
                ...
                If dsCliente.Tables(0).Rows.Count = 0 Then
                    ...
                    ls_insert = "insert into clientes (idCalle, nombre, poliza, numInterior, entreCalles) " & _
                ...
                Else
                    ...
                    Dim ls_update As String = "update clientes set  entreCalles=" & entreCalles & ", nombre=" & cliente & ", numInterior=" & numInterior & " where idCliente=" & dsCliente.Tables(0).Rows(0).Item("idCliente")
                    Try
                ...
                ls_insert = "insert into ordenes(numOrden, poliza, numMedidor, fechadeRecepcion, idCliente, tipodeorden,  idArchivo) " & _
                    " values (" & numOrden & ", " & poliza & ", " & numMedidor & ", str_to_date('" & getFecha("yyyyMMddHHmmss") & "', '%Y%m%d%H%i%s'), " & idCliente & ", " & tipoDeOrden & ", " & index & ")"
    ...
    Protected Sub bEnviar_Click(sender As Object, e As EventArgs) Handles bEnviar.Click
        ...
        Try
            ...
            '************************************************************************************************************************************************************************************************************
            ' CE, 19/08/16, Aparentemente solo necesitamos el idOrden, asi que quitamos todas las otras tablas
            selectSQL = "select ord.idOrden idOrden, if(ord.estadoDeLaOrden='EO001', true, false) asignar " & _
                        " from tareas tar, ordenes ord " & _
                        ...
                        " order by ord.idTarea, ord.numOrden "
            ...
            '                        " and ord.enviada='EE002' and tar.enviada ='EE002'  and ord.estadodelaorden not in ('EO002','EO003','EO004','EO005','EO006','EO007', 'CD000','CD001','CD002' ) " 
            ...
            For Each row In ds.Tables(0).Rows
                ...
                ' CE, 19/08/16, Vamos a sacar las siguientes tres lineas del ciclo
                ls_update = "update ordenes set enviada='EE001' where idOrden = " & row("idOrden")
                cmd.CommandText = ls_update
                cmd.ExecuteNonQuery()
            Next
            ' CE, 19/08/16, Estas tres lineas estaban dentro del ciclo
