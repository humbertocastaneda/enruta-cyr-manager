...
Partial Class verOrdenesPorEmpleado
    ...
    Public Sub LinkButton_Click(sender As Object, e As EventArgs)
        'Dim queryString = selectSQL.Replace("+", "%2B")
        Response.Redirect("verOrdenes.aspx?busqueda=" & CType(sender, LinkButton).Text & "&tipo=" & "ord.numOrden&q=" & selectSQL & _
    ...
    Public Sub getOrdenes()
        ...
        Try
            ...
            If (rbTipoDeBusqueda.SelectedValue = 0) Then 'Todas las que no se han empezado
                selectSQL = "Select cli.clave_usuario,cli.colonia, cli.direccion,ord.idlectura numOrden, if (ord.tipolectura='0', 'Con lectura', if(ord.tipolectura='4', 'Sin lectura', 'Pendiente')) estadoDeLaOrden, if(isnull(emp.alias) or LENGTH(emp.alias)=0, CONCAT_WS(' ',emp.nombre,  emp.appat, emp.apmat), emp.alias) tecnico,  ord.poliza poliza, ord.idlectura,ord.serieMedidor, ord.lectura, mot.nombre motivo , ord.fecha fechadeejecucion, fechaderecepcion, ord.comentarios, ord.latitud, ord.longitud " & _
                                      " , cli.sectorCorto, cli.Nombre cliente, cli.poliza  from lecturas ord left outer join  tareaslect tar on ( tar.idtarea=ord.idtarea) left outer join codigos mot on (ord.anomalia=mot.codigo), empleadoslect emp, clienteslect cli " & _
            ...
            'Hay que agregarle la columna fotos
            dsOrdenes.Tables("lecturas").Columns.Add("Foto")

            'hay que agregar foto por foto
            For Each row As DataRow In dsOrdenes.Tables("lecturas").Rows
                Dim nombreGenerico As String
                Dim extension As String = ".jpg"
                Dim ls_poliza As String = row("clave_usuario")
                Dim serieMedidor = row("serieMedidor").ToString.PadLeft(obtieneLongitud(conn, "serieMedidor"), "0")
                Dim ls_fechaDeEjecucion As String = row("fechadeejecucion").ToString
                Dim serieMedidorRaw = row("serieMedidor").ToString
                ...
                Dim nombreFinal As String = mostrarImagenes(Server.MapPath("~/") & "fotos\", "*" & ls_poliza & "-*" & serieMedidorRaw & "-" & fechaFinal, ls_poliza & "-*" & serieMedidor)

                row("Foto") = nombreFinal
            Next
    ...
    Public Sub generaArchivoPuntos(ds As DataSet, nombreArchivo As String)
        ...
        b_verMapa.Attributes.Add("onclick", "window.open('http://maps.google.com/maps?q=http://www.sistoleweb.com/tampico2/uploads/" & nombreArchivo & "." & ext & "'" & "); return false;")
    ...
    Microsoft.VisualBasic.FileIO.SearchOption.SearchAllSubDirectories, nombreGenerico & "*.jpg")
            ...
            Return String.Format("~/fotos/{0}", System.IO.Path.GetFileName(foundFile))
        Next
