...
Partial Class _Default
    ...
    Sub getResumen()
        ...
        Dim sqlSelect As String = "select foto,  calle, colonia, serieMedidor, esferas, lectura, comentarios " & _
        " from noregistrados " & _
        " where fechaDeIngresoAlSistema between str_to_date('" & fecha1.Text & "', '%Y%m%d') and  str_to_date('" & fecha2.Text & "', '%Y%m%d') order by calle, colonia"
        ...
        Try
            Dim ds As DataSet = conectarMySql(conn, sqlSelect, "noregistrados", True)

            For Each row As DataRow In ds.Tables("noregistrados").Rows
                If IsNothing(("foto")) Then
                    row("foto") = "botones/medFace.png"
                    Continue For
                End If

                If My.Computer.FileSystem.FileExists(Server.MapPath("~/") & "fotos\" & row("foto")) Then
            ...
            dgOrdenes.DataSource = ds.Tables("noregistrados")
            dgOrdenes.DataBind()
    ...
    Protected Sub bAExcel_Click(sender As Object, e As EventArgs) Handles bAExcel.Click
        Dim nombreArchivo As String = "No_Registrados_" & fecha1.Text & "_" & fecha2.Text
