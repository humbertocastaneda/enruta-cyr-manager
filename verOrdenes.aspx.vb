...
Partial Class _Default
    ...
    Public ls_poliza, ls_motivo, ls_anomalia, ls_queryString, ls_fechaDeEjecucion, _
    ...
    Public ls_longitud, ls_latitud, ls_estadoDeLaOrden, ls_descEstadoRev, ls_estadoDeLaRevision, ls_economico, ls_idTarea, ls_claveUsuario, ls_sectorCorto, ls_idCliente As String
    ...
    Public Sub setDatos(busqueda As Integer)
        ...
        Dim sqlSelect As String = "Select cli.idcliente, cli.sectorcorto, cli.clave_usuario, tar.idTarea idtarea, lec.poliza eco,  lec.idLectura, lec.tipoLectura, lec.fecha, lec.latitud, lec.longitud, lec.idlectura idlectura,  cli.poliza poliza,cli.direccion,  lec.fechaderecepcion fechaderecepcion, " & _
                        "lec.lectura lectura, lec.anomalia claveAnomalia, lec.serieMedidor serieMedidor, lec.comentarios comentarios, concat_ws(' ', emp.Nombre, emp.ApPat, emp.ApMat) lecturista,cli.nombre, mo.nombre anomalia from " & _
        ...
        Try
            ...
            ls_sectorCorto = row("sectorcorto")
    ...
    Public Sub setImagenes(conn As MySqlConnection)
        ...
        nombreGenerico = "*" & ls_poliza & "-*" & serieMedidor & "*" & fechaFinal

        mostrarImagenes(Server.MapPath("~/") & "fotos\", nombreGenerico, serieMedidor & "-" & fechaFinal, ls_poliza & "-*" & serieMedidor)
    ...
    Sub obtieneDSListado()
        ...
        Try
            ...
            For Each row As DataRow In dsLista.Tables(0).Rows
                If Convert.ToInt32(row("idLectura")) = Convert.ToInt32(Request.QueryString("busqueda")) Then
                    lugarEnLista = lugar
                    Exit For
                End If
                lugar += 1
    ...
    Protected Sub b_guardaLectura_Click(sender As Object, e As EventArgs) Handles b_guardaLectura.Click
        ...
        Try
            conn = getConnection()
            Dim ls_update As String = "update lecturas set lectura='" & tb_lectura.Text.ToString & "' where idLectura = " & idlectura
            cmd.CommandText = ls_update
            cmd.Connection = conn
            cmd.ExecuteNonQuery()
    ...
    Protected Sub b_guardaAnomalia_Click(sender As Object, e As EventArgs) Handles b_guardaAnomalia.Click
        If tb_anomalia.Text.ToString.Length > 1 Then
            message_generico("La anomalia no puede ser mayor a 1 caracter")
        ...
        Try
            conn = getConnection()
            Dim ls_update As String = "update lecturas set anomalia='" & tb_anomalia.Text.ToString & "' where idLectura = " & idlectura
            cmd.CommandText = ls_update
            cmd.Connection = conn
            cmd.ExecuteNonQuery()
            message_generico("Se ha guardado la anomalia")
        Catch ex As Exception
            message_generico("Ocurrio un error:" & ex.Message)
            'Throw New Exception("Ocurrio un error:" & ex.Message)
        Finally
    ...
    Protected Sub b_guardaComentario_Click(sender As Object, e As EventArgs) Handles b_guardaComentario.Click
        If tb_comentario.Text.ToString.Length > 250 Then
            message_generico("El comentario no puede ser mayor a 250 caracteres")
            Return
        End If
        Dim conn As MySqlConnection
        Dim cmd As New MySqlCommand()
        Try
            conn = getConnection()
            Dim ls_update As String = "update lecturas set comentarios = '" & tb_comentario.Text.ToString & "' where idLectura = " & idlectura
            cmd.CommandText = ls_update
            cmd.Connection = conn
            cmd.ExecuteNonQuery()
            message_generico("Se ha guardado el comentario")
        Catch ex As Exception
            message_generico("Ocurrio un error:" & ex.Message)
            'Throw New Exception("Ocurrio un error:" & ex.Message)
        Finally
